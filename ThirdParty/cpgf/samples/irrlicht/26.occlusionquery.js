...
function start()
{
	...
	smgr.getGUIEnvironment().addStaticText("Press Space to hide occluder.", new irr.rect_s32(10,10, 200,50));

	/*
	Create the node to be occluded. We create a sphere node with high poly count.
	*/
	var node = smgr.addSphereSceneNode(10, 64);
	if (node)
	{
		node.setPosition(new irr.vector3df(0,0,60));
	...
	*/
	var plane = smgr.addMeshSceneNode(smgr.addHillPlaneMesh("plane", new irr.dimension2df(10,10), new irr.dimension2du(2,2)), null, -1, new irr.vector3df(0,0,20), new irr.vector3df(270,0,0));
	...
	*/
	...
	var lastFPS = -1;
	var timeNow = device.getTimer().getTime();
	var nodeVisible=true;

	while(device.run())
	{
		plane.setVisible(! IsKeyDown(irr.KEY_SPACE));
		...
		*/
		...
		if (lastFPS != fps)
		{
			var tmp = "cpgf Irrlicht JavaScript binding OcclusionQuery Example [";
